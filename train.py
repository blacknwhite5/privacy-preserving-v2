...
def main():
    ...
    for epoch in range(num_epoch):
        for i, (img, masked_img, cls, landm) in enumerate(dataloader):
            img = img.to(device)
            ...
            loss_D_real = loss_MSE(D_real, torch.ones_like(D_real))
            loss_D_fake = loss_MSE(D_fake, torch.zeros_like(D_fake))
            loss_D_cls_real = loss_CE(D_real_cls, cls)

            loss_D = 0.5 * (loss_D_real + loss_D_fake) + loss_D_cls_real
            ...
            # Generator
            # # # # #
            loss_G_fake = loss_MSE(D_fake, torch.ones_like(D_fake))
...
#            loss_G_cls_fake = 15 - loss_CE(D_fake_cls, cls) 
            ...
            print("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f]" % \
                  (epoch, num_epoch, i*batch_size, len(celeba), loss_D.item(), loss_G.item()))

            # 이미지 저장
            if i % (20000//batch_size) == 0:
                save_image(torch.cat([img, fake], dim=3), 
                           f'images/ep{epoch+1:03d}_iter{i:05d}.png', 
                           nrow=batch_size//4, normalize=True)
